...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.MHAttn = self.MultiHeadedSelfAttention(model_dim, num_heads)
        self.VanilNN = self.VanillaNeuralNetwork(model_dim)
        self.LNs = nn.ModuleList([nn.LayerNorm(model_dim), nn.LayerNorm(model_dim)])
        # pass
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        norm1 = self.LNs[0](embedded)
        attn_out = self.MHAttn(norm1)
        x = embedded + attn_out

        norm2 = self.LNs[1](x)
        ffn_out = self.VanilNN(norm2)
        out = x + ffn_out
